...
export async function detectProject(root, opts = {}) {
    if (!existsSync(root)) {
    ...
    const mergedStateBeforeProbe = pkgResult.state ?? [];
    const stateResult = probeState(root, mergedStateBeforeProbe);
    const mergedUiBeforeProbe = pkgResult.ui ?? { componentLib: "none" };
    const componentsResult = probeComponents(root, mergedUiBeforeProbe);
    // Merge in priority order (later wins for the same key)
    const merged = mergeProbes(pkgResult, configsResult, routesResult, apiResult, stateResult, componentsResult);
    // Ensure required fields have safe defaults
    const profile = ensureRequired(merged, root);
...
function mergeProbes(...probes) {
    ...
    for (const probe of probes) {
        if (probe.stack)
            out.stack = { ...(out.stack ?? {}), ...probe.stack };
        if (probe.dev)
            out.dev = { ...(out.dev ?? { command: "", port: 3000, readyTimeoutMs: 60000, env: {} }), ...probe.dev };
        if (probe.routes)
            out.routes = { ...(out.routes ?? { type: "none", entries: [] }), ...probe.routes };
        if (probe.api)
            out.api = { ...(out.api ?? { surface: "none" }), ...probe.api };
        if (probe.state)
            out.state = probe.state;
        if (probe.ui)
            out.ui = { ...(out.ui ?? { componentLib: "none" }), ...probe.ui };
        if (probe.smoke)
            out.smoke = probe.smoke;
        if (probe.existing_e2e)
            out.existing_e2e = { ...(out.existing_e2e ?? {}), ...probe.existing_e2e };
    }
    return out;
}
function ensureRequired(p, root) {
    const stack = p.stack ?? { framework: "unknown", is_monorepo: false };
    const dev = p.dev ?? { command: "echo no-dev-command", port: 3000, readyTimeoutMs: 60000, env: {} };
    const routes = p.routes ?? { type: "none", entries: [] };
    const api = p.api ?? { surface: "none" };
    const state = p.state ?? [{ library: "none", stores: [] }];
    const ui = p.ui ?? { componentLib: "none" };
    // Default smoke URL uses the dev port
    const smoke = p.smoke ?? { url: `http://localhost:${dev.port}` };
    return {
        schema_version: "4.0",
        stack,
        dev,
