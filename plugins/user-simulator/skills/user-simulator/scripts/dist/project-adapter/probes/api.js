...
function detectIpcStyle(root) {
    ...
    for (const candidate of preloadCandidates) {
        ...
        if (/Hono<.*>|honoClient|hc<typeof/.test(text))
            return "hono-daemon";
        if (/window\.electron\.invoke/.test(text) && !/window\.api\./.test(text))
            return "generic-bridge";
        if (/window\.api\b/.test(text))
            return "typed";
    }
    return "none";
}
function readPackageLite(root) {
    try {
