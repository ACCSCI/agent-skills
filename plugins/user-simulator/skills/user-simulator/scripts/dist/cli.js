...
import { detectFromSignals, visionFindingsToInputs } from "./bug-detect/bug-detector.js";
...
async function loadJsonSchema(path) {
    return JSON.parse(await readFile(path, "utf8"));
}
async function validateCommand(flags) {
    const ajv = new Ajv2020.default({ allErrors: true, strict: false });
    addFormats.default(ajv);
    const personaSchema = await loadJsonSchema(resolve(import.meta.dirname, "../schemas/persona.schema.json"));
    const storySchema = await loadJsonSchema(resolve(import.meta.dirname, "../schemas/story.schema.json"));
    let ok = true;
    if (flags.persona) {
        const persona = parseYaml(await readFile(flags.persona, "utf8"));
    ...
    if (flags.story) {
        const story = parseYaml(await readFile(flags.story, "utf8"));
...
async function detectCommand(flags) {
    ...
    if (flags.signals) {
        ...
        for (const line of lines) {
            ...
            if (obj.kind === "infra") {
                const cls = classifyInfra({ errorMessage: typeof obj.entry === "object" && obj.entry && "message" in obj.entry ? String(obj.entry.message) : String(obj.entry) });
            ...
            inputs.push({
                ...
                confidence: "medium",
                type: "functional",
                title: String(obj.entry?.title ?? obj.kind),
            });
        }
    }
    if (flags.vision) {
        const vision = JSON.parse(await readFile(flags.vision, "utf8"));
...
async function reportCommand(flags) {
    ...
    if (!artifactRoot)
        throw new Error("--artifact-root is required");
    const store = new ArtifactStore({ artifact_root: artifactRoot });
    const bugsRaw = JSON.parse(await readFile(resolve(artifactRoot, "bugs.json"), "utf8"));
...
async function compareCommand(flags) {
    ...
    const artifactRoot = flags["artifact-root"];
    if (!baseline || !artifactRoot)
        throw new Error("--baseline and --artifact-root are required");
    const baseBugs = JSON.parse(await readFile(resolve(artifactRoot, "..", baseline, "bugs.json"), "utf8"));
    const curBugs = JSON.parse(await readFile(resolve(artifactRoot, "bugs.json"), "utf8"));
...
// ---------------------------------------------------------------------------
async function detectProjectCommand(flags) {
...
async function scaffoldCommand(flags) {
    const root = flags.target;
    if (!root) {
...
async function verifyApiCommand(flags) {
    const root = flags.target;
    ...
    const story = parseYaml(await readFile(storyPath, "utf8"));
    const allAssertions = [];
    for (const step of story.steps ?? []) {
        // When --step <id> is given, only run that step's assertions
        if (flags.step && step.id !== flags.step)
            continue;
    ...
    const results = await runner.runAll({ assertions: allAssertions.map((a) => a.assertion) });
    let pass = 0;
    let fail = 0;
...
function renderVerificationReport(diff) {
    const lines = [`# Round 2 Verification Report\n`, `\n**Verdict**: ${diff.verdict}\n`, `\n**Reason**: ${diff.reason}\n`];
    ...
    if (diff.regression.length) {
        lines.push(`\n## Regression\n`);
        for (const t of diff.regression)
            lines.push(`- ${t.baseline_bug_id}: ${t.status}`);
    }
    if (diff.new_bugs.length) {
        lines.push(`\n## New Bugs\n`);
        for (const b of diff.new_bugs)
            lines.push(`- ${b.severity} ${b.title} (${b.bug_id})`);
    }
    if (diff.persistent.length) {
        lines.push(`\n## Persistent\n`);
        for (const b of diff.persistent)
            lines.push(`- ${b.severity} ${b.title} (${b.bug_id})`);
    }
    if (diff.inconclusive.length) {
        lines.push(`\n## Inconclusive\n`);
        for (const b of diff.inconclusive)
            lines.push(`- ${b.severity} ${b.title} (${b.bug_id})`);
    }
    return lines.join("\n");
}
...
async function runSubcommand(sub, flags) {
    switch (sub) {
