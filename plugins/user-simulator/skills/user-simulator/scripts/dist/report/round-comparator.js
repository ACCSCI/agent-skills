 ...
 */
export function compareRounds(baselineBugs, currentBugs, baselineRunId) {
    const baselineByFp = new Map();
    for (const b of baselineBugs)
        baselineByFp.set(b.fingerprint, b);
    const currentByFp = new Map();
    for (const b of currentBugs)
        currentByFp.set(b.fingerprint, b);
    const targeted = [];
    const persistentBugs = [];
    const inconclusive = [];
    const newBugs = [];
    const regression = [];
    ...
    const persistentS0S1 = [
        ...targetedPersistent.map((t) => baselineByFp.get(t.baseline_fingerprint)).filter((b) => !!b && (b.severity === "S0" || b.severity === "S1")),
    ...
    return {
        ...
        regression,
        new_bugs: newBugs,
        persistent: targetedPersistent.map((t) => baselineByFp.get(t.baseline_fingerprint)).filter((b) => !!b),
        inconclusive,
        verdict,
        reason,
    };
...
//# sourceMappingURL=round-comparator.js.map