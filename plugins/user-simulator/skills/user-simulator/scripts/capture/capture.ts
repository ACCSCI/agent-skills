...
import type { Page, ElectronApplication, BrowserContext } from "@playwright/test";
import type {
  ConsoleEntry,
  MetricsSample,
  PageErrorEntry,
...
import { fingerprint, type FingerprintableState } from "../persona-loop/state-fingerprint.js";
...
export interface BoundCapture {
  ...
  page_error_buffer: PageErrorEntry[];
  request_failure_buffer: RequestFailureEntry[];
}

export function bind(page: Page): BoundCapture {
  const console_buffer: ConsoleEntry[] = [];
  const page_error_buffer: PageErrorEntry[] = [];
  const request_failure_buffer: RequestFailureEntry[] = [];
...
export function drain(bound: BoundCapture): SignalDeltas {
  return {
...
export async function captureSnapshot(bound: BoundCapture, opts: CaptureOptions = {}): Promise<Capture> {
  ...
  const fp_input: FingerprintableState = {
    url,
    title,
    headings,
    landmarks,
  ...
  const capture: Capture = { state, signals: drain(bound) };
...
export async function takeScreenshot(bound: BoundCapture, mode: "viewport" | "full_page" = "viewport"): Promise<Buffer> {
  return bound.page.screenshot({ fullPage: mode === "full_page", type: "png" });
}

// ---- helpers ----

async function safeHeadings(page: Page): Promise<string[]> {
...
async function safeLandmarks(page: Page): Promise<{ role: string; label?: string }[]> {
  return page.evaluate(() =>
    Array.from(document.querySelectorAll("header, nav, main, aside, footer, [role='banner'], [role='navigation'], [role='main'], [role='complementary'], [role='contentinfo']"))
      .slice(0, 50)
      .map((el) => ({
...
async function safeDialogs(page: Page): Promise<{ role: string; label?: string }[]> {
  return page.evaluate(() =>
    Array.from(document.querySelectorAll("dialog, [role='dialog'], [role='alertdialog']"))
      .slice(0, 20)
      .map((el) => ({
        role: el.getAttribute("role") ?? "dialog",
        label: (el.getAttribute("aria-label") ?? el.getAttribute("aria-labelledby") ?? undefined) as string | undefined,
      })),
  ).catch(() => []);
}

async function safeControlCount(page: Page): Promise<number> {
  return page.evaluate(() => {
...
async function safeMetrics(page: Page): Promise<MetricsSample> {
  return page.evaluate(() => {
    const navEntry = performance.getEntriesByType("navigation")[0] as PerformanceNavigationTiming | undefined;
    const paints = performance.getEntriesByType("paint");
    const fcp = paints.find((p) => p.name === "first-contentful-paint")?.startTime;
    const lt = performance.getEntriesByType("longtask") as PerformanceEntry[];
...
export async function getFirstPage(app: ElectronApplication): Promise<Page> {
  return app.firstWindow({ timeout: 30_000 });
}

/** Helper for round-2 storage state reuse. */
export async function snapshotStorageState(ctx: BrowserContext): Promise<unknown> {
