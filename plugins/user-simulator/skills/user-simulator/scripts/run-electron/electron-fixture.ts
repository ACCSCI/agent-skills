...
import { bind, type BoundCapture } from "../capture/capture.js";

export interface ElectronLaunchArgs {
...
export interface ElectronHandle {
  ...
  window: Page;
  bound: BoundCapture;
  crashedBeforeWindow: boolean;
}

export async function launchElectron(args: ElectronLaunchArgs): Promise<ElectronHandle> {
  const launchOpts = {
...
export async function teardownElectron(handle: ElectronHandle): Promise<void> {
  try {
...
export function isCrashed(handle: ElectronHandle): boolean {
