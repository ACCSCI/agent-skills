...
import { PersonaLoop, type PersonaLoopInit } from "./persona-loop.js";
import type { ActionRequest, Assertion, Locator } from "../common/contracts.js";

function parseArgs(argv: string[]): { verb: string; flags: Record<string, string> } {
  const verb = argv[0];
...
function buildLoop(): PersonaLoop {
  const artifact_root = envOrThrow("USER_SIM_ARTIFACT_ROOT");
  const platform = envOrThrow("USER_SIM_PLATFORM") as "web" | "electron";
  const opts: PersonaLoopInit = {
    artifact_root,
    platform,
...
async function main(): Promise<void> {
  ...
  const req: ActionRequest = {
    verb: verb as ActionRequest["verb"],
    ...(flags.target ? { target: flags.target } : {}),
    ...(flags.locator ? { locator: flags.locator as Locator } : {}),
    ...
    ...(flags.assertion ? { assertion: JSON.parse(flags.assertion) as Assertion } : {}),
  };
