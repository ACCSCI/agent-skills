...
const COMPONENT_LIBS: Array<{ lib: string; keys: string[] }> = [
...
const STATE_LIBS: Array<{ lib: string; keys: string[] }> = [
...
const ANY_DEPS = (...deps: string[]) => (pkg: PackageJson) =>
  [...Object.keys(pkg.dependencies ?? {}), ...Object.keys(pkg.devDependencies ?? {})].some((d) => deps.includes(d));

const hasAny = (keys: string[]) => (pkg: PackageJson) =>
  [...Object.keys(pkg.dependencies ?? {}), ...Object.keys(pkg.devDependencies ?? {})].some((d) => keys.includes(d));

export async function probePackage(root: string): Promise<ProbeContribution> {
  ...
  if (ANY_DEPS(...NEXT_DEPS)(pkg)) {
    ...
    if (ANY_DEPS("electron-vite")(pkg)) out.stack.framework = "electron-vite";
    else if (ANY_DEPS("vite-plugin-electron")(pkg)) out.stack.framework = "electron-vite-plugin";
    else if (ANY_DEPS("electronmon", "esbuild")(pkg)) out.stack.framework = "electron-esbuild";
    else out.stack.framework = "electron-vite";
  ...
  for (const { lib, keys } of COMPONENT_LIBS) {
    if (hasAny(keys)(pkg)) {
      out.ui = { componentLib: lib as ProbeContribution["ui"] extends infer U ? U extends { componentLib: infer C } ? C : never : never };
      break;
    }
  }
  ...
  for (const { lib, keys } of STATE_LIBS) {
    if (hasAny(keys)(pkg)) {
      out.state.push({ library: lib as never, stores: [] });
    }
  }
...
export function extractPort(command: string): number | undefined {
  ...
  if (!last || last[1] === undefined) return undefined;
