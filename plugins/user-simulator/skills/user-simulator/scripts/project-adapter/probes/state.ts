...
import type { ProbeContribution, StateInfo } from "../types.js";

const MAX_STORES = 20;

export function probeState(root: string, fromPkg: StateInfo[]): ProbeContribution {
...
function listStoreFiles(dir: string): string[] {
  ...
  try {
    for (const entry of readdirSync(dir)) {
      if (!/\.(ts|js|tsx|jsx)$/.test(entry)) continue;
