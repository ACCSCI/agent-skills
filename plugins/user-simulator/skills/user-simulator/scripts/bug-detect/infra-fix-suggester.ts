...
import type { InfraClassification } from "./failure-classifier.js";

export interface FixSuggestion {
  title: string;
  description: string;
  files: Array<{ path: string; change: "modify" | "create" | "delete"; rationale: string }>;
  confidence: number;
  codeSnippet?: string;
}
...
export function suggestInfraFix(classification: InfraClassification): FixSuggestion[] {
...
export function formatInfraFixReport(suggestions: FixSuggestion[]): string {
  if (suggestions.length === 0) return "No infra fix suggestions.";
  const lines: string[] = ["## Infra Fix Suggestions\n"];
